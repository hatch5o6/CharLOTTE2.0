...
def read_oc_data(path):
    ...
    assert tuple_size in [4, 5], f"OC tuples must be of size 4 (freq, w1, w2, dist) or 5 (freq1, freq2, w1, w2, dist). Got {tuple_size}!"

    data = []
    for line in lines:
        assert len(line) == tuple_size, f"Inconsistent tuple sizes! Should be {tuple_size} but got {len(line)}!"
